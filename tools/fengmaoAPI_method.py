...
from tools.fengmao_md5 import md5encryption
from tools.fengmao_md5 import oldMd5sign
from tools.mysql_db import DB
from tools.OpenExcel import openExcel
from tools.httpRequest import httpRequest
from tools.http_request import http_req
from tools.fengmao_redis import *
...
class fengmaoAPI_method:
    ...
    def order_confirm(self, caseParamData):
        ...
        while 1:
            if fengmao_Global.get_value('mobile') != '':
                ...
                # 所有车票价格
                caseParam['orderAmount'] = str(float(dicTrain_Value['buyFee']) + float(dicTrain_Value['price']) * len(
                    caseParam['trainParams']['passengers']))
        ...
        while 1:  # 查询确认订单之后的占座结果，如果占座成功就直接支付，如果失败，再次占座
            ...
            if orderStatus == 2:
                ...
                for i in (range(0, len(orderDtl_result['result']['ticketList']))):
                    ticketPirce = ticketPirce + float(
                        orderDtl_result['result']['ticketList'][i]['ticketPrice']) + float(
                        orderDtl_result['result']['ticketList'][i]['buyFee'])
                orderAmount = float(orderDtl_result['result']['totalInsurePrice']) + ticketPirce + float(
                    orderDtl_result['result']['payPoundageAmount']) - float(
                    orderDtl_result['result']['orderDiscountAmount'])
                # 插入优惠券
                couponPrice = 0.0
                ...
                while 1:
                    ...
                    ticketSQL = Mysql_db.dbselect(
                        "SELECT * FROM train_ticket WHERE ORDER_CODE = " + "'" + crteatOrderDic[
                            'orderCode'] + "'" + " and TICKET_STATUS = 4 AND PASSENGER_TYPE =1;")
                    Mysql_db.close()
                    if len(ticketSQL) != 0:
                        # print(ticketSQL)
    ...
    def train_ticket_applyRefundOrder(self, orderCode):  # 火车票申请退票接口
        ...
        caseparam = self.baseConfig.getParamByCaseID(fname, sheetName, '1')
        caseparam['appType'] = fengmao_Global.get_value('appType')
        Refund_tarin_order_list = self.Mysql_db.dbselect(
            "SELECT * FROM train_ticket WHERE ORDER_CODE = " + "'" + orderCode + "'" + " and TICKET_STATUS = 4 ")
    ...
    def train_contact_findList(self):  # 火车票常用联系人查询接口
        ...
        Contact_param = {'submitTime': '20160714090000', 'appVersion': '2.1.3',
                         'appType': fengmao_Global.get_value('appType'), 'osname': 'IOS',
                         'mobile': fengmao_Global.get_value('mobile')}
        req_json = self.http_request.http_req_type(Contact_param, internfaceName, '0')
            ...
            if req_json['errCode'] == '10000':
                if caseParam['passengerType'] == '1':
                    if "-" not in caseParam['birthday']:
                        caseParam['birthday'] = time.strftime("%Y-%m-%d",
                                                              time.strptime(caseParam['birthday'], "%Y%m%d"))
                ...
                if dicOrderInfo['orderType'] == '1':
                    ...
                    if dicOrderInfo['changeType'] == '1':  # dicOrderInfo['changeType']: 1:低改高。2：平价改。3高改低
                        for trains in req_json['result']['trains']:
                            # print trains
                            for tickets in trains['tickets']:
                                if float(tickets['seats']) > 0 and (
                                        float(tickets['price']) > float(dicOrderInfo['ticketPrice'])):
                                    dicSeat['trainNo'] = trains['trainNo']
                        ...
                        for trains in req_json['result']['trains']:
                            # print trains
                            for tickets in trains['tickets']:
                                if float(tickets['seats']) > 0 and float(tickets['price']) == float(
                                        dicOrderInfo['ticketPrice']):
                                    dicSeat['trainNo'] = trains['trainNo']
                                    dicSeat['startStation'] = trains['fromStationCode']
                                    dicSeat['fromStation'] = trains['fromStation']
                                    dicSeat['toStation'] = trains['toStation']
    ...
    def insert_ECcoupon(self, userid, couponPrice, userType):
        inserSQL = ("INSERT INTO `ec_coupon_user` "
                    ...
                    "VALUES "
                    "(" + "'" + str(userid) + "'" + ", '26', 'act_buy_train_1', '36', '一元抢购', " + "'" + str(
            couponPrice) + "'" + ", '0', '2', 'DEDUC_COUPON', " + "'" + str(
            userType) + "'" + ", '2017-08-03 15:56:26', '2017-08-03 15:56:26', '2018-09-02 15:56:26', NULL, NULL, '1', '1', NULL, NULL, '1');")
        self.Mysql_db.dbInsert(inserSQL)
        selectSQL = "SELECT REC_ID FROM ec_coupon_user WHERE USER_ID = " + str(
            userid) + " AND USE_TIME IS NULL AND STATUS = 1 AND COUPON_ID = 36 AND DISCOUNT_AMOUNT = " + str(
            couponPrice)
            ...
            if result['success'] == True:
                ...
                if APIdicparam['appType'] == 'M':
                    payPwd = My_sql.dbselect(
                        "SELECT PAY_PASSWORD FROM mem_user WHERE LOGIN_ACCOUNT =" + "'" + mobile + "'" + " AND SUB_USER_TYPE ='2' ")[
                        0]['PAY_PASSWORD']
                    ...
                    payPwd = My_sql.dbselect(
                        "SELECT PAY_PASSWORD FROM mem_user WHERE LOGIN_ACCOUNT =" + "'" + mobile + "'" + " AND SUB_USER_TYPE ='4'")[
                        0]['PAY_PASSWORD']
                    userID = My_sql.dbselect(
                    ...
                    payPwd = My_sql.dbselect(
                        "SELECT PAY_PASSWORD FROM mem_user WHERE LOGIN_ACCOUNT =" + "'" + mobile + "'" + " AND SUB_USER_TYPE ='3' ")[
                        0]['PAY_PASSWORD']
                    userID = My_sql.dbselect(
